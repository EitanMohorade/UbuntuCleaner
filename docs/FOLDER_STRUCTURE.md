...
Inspecciona el estado después de ejecución:

```bash
# Ver todo el reporte (requiere jq)
jq . state/last_run.json

# Contar eventos de cada tipo
jq '.report_counts' state/last_run.json

# Ver sólo eventos OK
jq '.report_events.ok[]' state/last_run.json

# Ver qué se liberó realmente
jq '.space_freed_human' state/last_run.json
```
